...
int main()
{
	...
	for(int i = 0; i < 2; ++i){ Waterloo.add_agent(Person('C', 5, 'S', 0, 0)); }
	for(int i = 0; i < 2; ++i){ Waterloo.add_agent(Person('C', 6, 'S', 0, 0)); }
	...
	for(int i = 0; i < 2; ++i){ Waterloo.add_agent(Person('C', 7, 'S', 0, 0)); }
	Waterloo.add_agent(Person('A', 7, 'S', -1));

	// Houses to pull substitutes from
	for(int i = 0; i < 2; ++i){ Waterloo.add_agent(Person('C', 1, 'S', -1)); }
	for(int i = 0; i < 2; ++i){ Waterloo.add_agent(Person('C', 2, 'S', -1)); }
	for(int i = 0; i < 2; ++i){ Waterloo.add_agent(Person('C', 3, 'S', -1)); }
	for(int i = 0; i < 2; ++i){ Waterloo.add_agent(Person('A', 1, 'S', -1)); }
	for(int i = 0; i < 2; ++i){ Waterloo.add_agent(Person('A', 2, 'S', -1)); }
	for(int i = 0; i < 2; ++i){ Waterloo.add_agent(Person('A', 3, 'S', -1)); }

	for(int i = 0; i < 4; ++i){ Waterloo.add_agent(Person('C', 8, 'S', 2, 0)); }
	Waterloo.add_agent(Person('A', 9, 'A', 2, 0));
	Waterloo.add_agent(Person('A', 9, 'A', -1));

	Waterloo.add_agent(Person('A', 10, 'S', -1));
	Waterloo.add_agent(Person('A', 10, 'S', -1));
	Waterloo.add_agent(Person('A', 10, 'S', -1));
	Waterloo.add_agent(Person('A', 10, 'S', -1));
	...
	Waterloo.set_status(19, 'I');
	Waterloo.print_households();
	printf("\nCHECK: printing the susceptible agents in the school currently. It's a weekend: %i, so no one is in school right now.\n", Waterloo.currently_the_weekend());
	std::cout << Waterloo.agents_in_school({'S'}) << std::endl;
	...
	Waterloo.set_status(5, 'R'); 	Waterloo.set_status(1, 'R');
	for(int i=0; i<9; ++i){ Waterloo.advance_the_time(); }
	// printf("\nHouseholds:");
	// Waterloo.print_households();
	printf("\nClassrooms:\n");
	Waterloo.print_classrooms();

	printf("\nCHECK: Proportion of asymptomatic (A) agents in the school: %f. Should be 1/12 now.\n", Waterloo.agents_in_school_proportion('A'));

	printf("\n\nHit ENTER to continue.\n\n"); getchar();

	printf("\nTEST: Class 2 is fine. Advancing 7 days, and child 7 should be recovered and back in class 0.\n");
	for(int i=0; i<8; ++i){ Waterloo.advance_the_time(); }
	Waterloo.print_classrooms();
	...
	Waterloo.set_status(22, 'I');
	Waterloo.print_classroom(2);

	Waterloo.advance_the_time();
	printf("\nCHECK: Child has had a day to run around and spread SARS-CoV-2. It's a new day now, but it's the weekend: %i, for day %i of the sim. Printing the classrooms:\n", Waterloo.currently_the_weekend(), Waterloo.days_elapsed());
	Waterloo.print_classrooms();

	printf("\n\nHit ENTER to continue.\n\n"); getchar();

	Waterloo.advance_the_time(); printf("\tStill the weekend..."); Waterloo.print_classrooms();

	Waterloo.advance_the_time();
	printf("\nCHECK: Not the weekend anymore: %i. Classroom 2 should be shut.", not Waterloo.currently_the_weekend());
	Waterloo.print_classrooms();

	printf("\nCHECK: Number of classrooms shut down: %i.\n", (int)Waterloo.closed_classrooms().size());

	for(int i=0; i < 14; i++)
	{
		printf("\tAdvanced the time to day %i. Classroom 2 is closed for %i days. It's the weekend (true/false): %i.\n", Waterloo.days_elapsed(), Waterloo.classroom_closure_time(2), Waterloo.currently_the_weekend());
		Waterloo.advance_the_time();
	}

	printf("\n\nHit ENTER to continue.\n\n"); getchar();

	printf("\nTEST: We've zoomed ahead 14 days so both classrooms should be opened now on day %i, with child 22 recovered. The line with 14 days repeated twice above because those last 2 days were weekend days (so class would not be open anyway).", Waterloo.days_elapsed());
	Waterloo.set_status(22, 'R');
	Waterloo.print_classrooms();
